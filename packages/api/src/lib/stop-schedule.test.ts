...
describe("parseSchedulesResponse", () => {
  ...
  it("turns a real-time arrival into now + offset, and a scheduled one into a clock time", () => {
    ...
    expect(scheduled[0]!.arrivalTime.getTime() - NOW.getTime()).toBe(
      112 * 60 * 1000,
    );
  });
  ...
  it("keeps each journey's arrivals sorted ascending", () => {
    ...
    for (const line of schedules) {
      for (const journey of line.journeys) {
        const times = journey.scheduledTimes.map((t) =>
  ...
  it("parses two real-time buses with correct offsets and accessibility", () => {
    ...
    const offsets = journey.scheduledTimes.map(
      (t) => t.arrivalTime.getTime() - NOW.getTime(),
    );
  ...
  it("marks an all-scheduled (night) response with no real-time arrivals", () => {
    ...
    const allTimes = schedules.flatMap((s) =>
      s.journeys.flatMap((j) => j.scheduledTimes),
    );
  ...
  it("agrees with the countdown Moventis itself reported at capture time", () => {
    ...
    const journey = schedules.find((s) => s.externalLineId === "130")!
      .journeys[0]!;
  ...
  it("resolves scheduled times against the winter offset too", () => {
    ...
    expect(toWallClock(first)).toMatchObject({
      month: 1,
      day: 15,
      hour: 6,
      minute: 17,
  ...
  it("accepts the array-form trayectos edge case", () => {
    const schedules = parseSchedulesResponse(
      loadFixture("schedule-array-form.json"),
      NOW,
    );
  ...
  it("filters the sentinel response to an empty schedule list", () => {
    expect(
      parseSchedulesResponse(loadFixture("schedule-sentinel.json"), NOW),
    ).toEqual([]);
  });
  ...
  it("degrades one line with no ` - ` separator instead of dropping the whole stop", () => {
    ...
    const realtime = loadFixture("schedule-realtime.json") as Record<
      string,
      unknown
    >[];
    const mixed = loadFixture("schedule-mixed.json") as Record<
      string,
      unknown
    >[];
  ...
  it("throws ZodError on a malformed response (contract change)", () => {
    expect(() =>
      parseSchedulesResponse(loadFixture("schedule-malformed.json"), NOW),
    ).toThrow(ZodError);
  });
});
