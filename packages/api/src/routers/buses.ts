...
/**
 * How long one line's located positions are shared between callers. The
 * locator spends ~7–15 Moventis requests per variant, so every viewer of a line
 * refetching on their own 25 s timer would multiply that by the audience;
 * sharing one computation per line for a few seconds makes the probe budget a
 * per-line cost rather than a per-user one. Shorter than the client's refetch
 * interval, so consecutive refreshes of one client still see fresh reads.
 */
...
export const busesRouter = createTRPCRouter({
  ...
  byLine: publicProcedure
    ...
    .query(async ({ ctx, input }): Promise<BusPosition[]> => {
      ...
      value.catch(() => lineCache.delete(input.routeCode));
      return value;
    }),
});
