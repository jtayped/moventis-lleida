...
test("collapses a shared trunk shared by two variations", () => {
  ...
  const variations = [{ points: [A, B, C, D] }, { points: [A, B, C, E] }];
  ...
  assert.ok(
    outPoints < rawPoints,
    "deduped points should be fewer than raw sum",
  );

  // The shared trunk A-B-C must appear exactly once across all output paths.
  const flat = paths.flat();
  const countB = flat.filter(
    ([lng, lat]) => lng === B[0] && lat === B[1],
  ).length;
  ...
  const countC = flat.filter(
    ([lng, lat]) => lng === C[0] && lat === C[1],
  ).length;
