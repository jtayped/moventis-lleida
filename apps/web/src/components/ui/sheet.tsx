...
/**
 * A panel anchored to one edge of the window for the whole time it is open.
 *
 * `flex flex-col` with no scroller of its own: the header stays put and the body
 * a caller marks `min-h-0 flex-1 overflow-y-auto` is what moves. A sheet that
 * scrolls as a whole loses its title the moment the content is long, which is
 * the one thing it has over a dialog.
 */
...
function SheetHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex shrink-0 flex-col gap-1 p-6 pb-0", className)}
      {...props}
    />
  );
}
